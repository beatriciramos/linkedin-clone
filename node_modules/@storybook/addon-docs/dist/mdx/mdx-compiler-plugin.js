...
function genImportStory(ast, storyDef, storyName, context) {
  ...
  var storyKey = "".concat(story.split('.').pop(), "Story");
...
function genStoryExport(ast, context) {
  ...
  if (!bodyNodes.length) {
    // plain text node
    var _generate3 = generate(ast.children[0], {}),
    ...
    var bodyParts = bodyNodes.map(function (bodyNode) {
      ...
      var _generate4 = generate(body, {}),
          code = _generate4.code;
  ...
  if (parameters) {
    var _generate5 = generate(parameters, {}),
  ...
  if (decorators) {
    var _generate6 = generate(decorators, {}),
...
function genCanvasExports(ast, context) {
  ...
  for (var i = 0; i < ast.children.length; i += 1) {
    ...
    if (child.type === 'JSXElement' && child.openingElement.name.name === 'Story') {
      ...
      var _generate7 = generate(child, {}),
          code = _generate7.code;
  ...
  if (title) {
    if (title.type === 'StringLiteral') {
      ...
      try {
        ...
        var _generate8 = generate(title, {}),
            code = _generate8.code; // remove the curly brackets at start and end of code
...
function getExports(node, counter, options) {
  ...
  if (type === 'jsx') {
    if (STORY_REGEX.exec(value)) {
      ...
      var _generate9 = generate(ast, {}),
          code = _generate9.code; // eslint-disable-next-line no-param-reassign
    ...
    if (CANVAS_REGEX.exec(value)) {
      ...
      var canvasExports = genCanvasExports(_ast, counter);

      var _generate10 = generate(_ast, {}),
          _code = _generate10.code; // eslint-disable-next-line no-param-reassign
...
function stringifyMeta(meta) {
  var result = '{ ';
  Object.entries(meta).forEach(function (_ref4) {
...
function extractExports(node, options) {
  ...
  node.children.forEach(function (n) {
    ...
    if (exports) {
      ...
      if (stories) {
        Object.entries(stories).forEach(function (_ref6) {
  ...
  var defaultJsx = mdxToJsx.toJSX(node, {}, Object.assign(Object.assign({}, options), {}, {
    skipExport: true
  }));
